...
#' @param break_outer Stopping criterion for outer iterations
...
controlGIST <- function(){
  return(controlGIST <- list(
    ...
    "break_outer" = 10^(-5), # Stopping criterion for outer iterations
    ...
    "exactApproximateFirst" = "auto", # Should approximate optimization be used first to obtain start values for exact optimization? 1 and 2 are using OpenMx with 1 = optimization only for first regValue, 2 = optimization for all regValues. 3 ensures that the fit will not be worse than in the sparse model if regValues = "auto" or sparseParameters are provided. To this end, exactApproximateFirst3NumStartingValues + 2 models between the current parameter estimates and the sparse parameter estimates are tested and the one with the lowest regM2LL is used for starting values. 4 = optimizing using optim or OpenMx if cpptsem is not available, 5 = optimizing using Rsolnp or OpenMx if cpptsem is not available (requires installation of Rsolnp)
    "exactApproximateFirst3NumStartingValues" = 0, # Used if exactApproximateFirst = 3. regCtsem will try exactApproximateFirst3NumStartingValues+2 starting values (+2 because it will always try the current best and the parameters provided in sparseParameters)
    "exactApproximateFirst3Optimize" = 1, # Used if exactApproximateFirst = 3. Should each of the generated starting values be optimized slightly? This can substantially improve the fit of the generated starting values. 1 = optimization with optim, 2 = optimization with Rsolnp
    "exactApproximateFirstMaxIter_out" = "auto", # Used if exactApproximateFirst = 3 and exactApproximateFirst3Optimize > 1. How many outer iterations should be given to each starting values vector? More will improve the selected starting values but slow down the computation. If exactApproximateFirst =  4, or exactApproximateFirst = 5 this will control the number of outer iteration in optim or solnp .

...
Finally, **regCtsem** makes use of **RcppArmadillo** which allows for parallelization with OMP. Depending on how you installed **RcppArmadillo**, you might be able to take advantage of multiple cores. However, we currently recommend manually setting the number of threads: 
```{r, eval = FALSE}
Sys.setenv(OMP_NUM_THREADS="1")
```
Depending on the analysis at hand, using multiple cores can add a considerable overhead to the computation and we therefore currently recommend using only a single core.

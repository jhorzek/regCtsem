...
The results can be plot with the generic plot function:
...
```
Now the fit for the cross-validation is given in regularizedModel:

```{r}
regularizedModel$fit[,1:5]
```
Finally, note that **regCtsem** internally translates the models with **RcppArmadillo** to C++ with regCtsem::cpptsemFromCtsem (see ?cpptsemFromCtsem). Currently, some specific model settings from **ctsemOMX** will not be available in **regCtsem**. 
...
If a model is fitted with Kalman filter in **ctsemOMX** and then passed to **regCtsem**, **regCtsem** will also use the Kalman filter (don't run the following code, it will take a very long time):
...
Extracting the best parameters from the regularized model can be achieved with 
```{r}
...
However, when loading the model in the next session, the underlying C++ model will be lost. This model can restored, however:

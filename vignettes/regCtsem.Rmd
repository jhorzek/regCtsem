...
```
Finally, note that **regCtsem** internally translates the models with **RcppArmadillo** to C++ with regCtsem::cpptsemFromCtsem (see ?cpptsemFromCtsem). Currently, some specific model settings from **ctsemOMX** will not be available in **regCtsem**. **regCtsem** will throw an error if it encounters such cases.

## LASSO and adaptive LASSO

A difficulty when using the LASSO is that the same $\lambda$ is used for all regularized parameters. This is only justified if the regularized parameters have the same units (i.e., they all have the same scale). In latent variable models this is difficult to control. Therefore, **regCtsem** offers an approximate standardization only. To this end, the T0-variance or the asymptotic diffusion can be used:
...
Extracting the best parameters from the regularized model can be done as follows:
```{r}
getFinalParameters(regCtsemObject = regModel, 
                   criterion = "AIC", 
                   raw = T) 
# raw means that the parameters are returned
# exactly in the form they are implemented in ctsemOMX and cpptsem. 
# If you set raw = F the diffusion and T0Var will be computed from
# the internal parameter specifications:
getFinalParameters(regCtsemObject = regModel, 
                   criterion = "AIC", 
                   raw = F) 
```

The best model can be returned as follows:
...
However, when loading the model in the next session, the underlying C++ model will be lost. This model can restored:
...
### Chosing parameters to regularize

In general, **regCtsem** can regularize any parameter in the model. If you are unsure which parameters these might be or what they are called in your model, use the following code:

```{r}
omxGetParameters(fit_myModel$mxobj) # general: fittedCtsemObject$mxobj
```

This function will show you the names and values of the parameters in the model.
...
plot(regModel)
```
Regularizing parameters towards specific targets can be used to model selected parameters person-specific while regularizing the towards the group value. This feature is still experimental, but can be used as follows:
